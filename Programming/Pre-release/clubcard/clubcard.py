...
def BuyerJoinsQ(Data, BuyerQ, QLength, BuyerNumber):
    ...
    count = 0
    # print("####################")
    # print(BuyerQ[count].BuyerID[1:2])
    # print("####################")
    if int(BuyerNumber) % 2 == 0 and QLength > 0:  # if the buyer arriving is club card member AND there is a GODDAMN queue!
        while int( BuyerQ[count].BuyerID[1:] ) % 2 == 0:
            count += 1   # count number of people at the front of the queue who are clubcard members
        # insert = count + 1  # index of inserting the new buyer
        for buyer in range(QLength, count - 1, -1):  # reverse. pull people from the back!
            BuyerQ[buyer + 1].BuyerID = BuyerQ[buyer].BuyerID
            BuyerQ[buyer + 1].ItemsInBasket = BuyerQ[buyer].ItemsInBasket
        print(f"#### COUNT:{count}")
        BuyerQ[count].BuyerID = f"B{BuyerNumber}"  # insert person in
        BuyerQ[count].ItemsInBasket = ItemsInBasket  # insert person in
    else: 
...
def QueueSimulator():
    BuyerNumber = 0
    QLength = 0  # initially queue length is set to zero.
