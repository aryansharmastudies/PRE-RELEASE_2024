...
def ChangeSettings():
  ...
  if Answer == 'Y': #if anything else entered, it opts to default settings i.e. 29 time
...
def BuyerJoinsQ(Data, BuyerQ, QLength, BuyerNumber):
  ...
  BuyerQ[QLength].BuyerID = f"B{BuyerNumber}"  # instantiates a buyer (BuyerID)
...
def ServeBuyer(BuyerQ, QLength): # USED TO MOVE EVERYONE UP THE QUEUE. 
  ...
  for Count in range(QLength):
    BuyerQ[Count].BuyerID = BuyerQ[Count + 1].BuyerID # FIRST BUYERS ID REPLACED BY 2ND BUYERS ID
    BuyerQ[Count].WaitingTime = BuyerQ[Count + 1].WaitingTime
    BuyerQ[Count].ItemsInBasket = BuyerQ[Count + 1].ItemsInBasket
  BuyerQ[QLength].BuyerID = BLANK  # SINCE EVERYONE MOVES UP, LAST BUYER ID REMOVED.
...
def UpdateStats(Stats, WaitingTime):
  ...
  if WaitingTime == 0:
    Stats[TOTAL_NO_WAIT] += 1 # IF THERE IS NO WAIT TIME FOR THE FIRST BUYER, THEN
    # TOTAL_NO_WAIT += 1, TOTAL_NO_WAIT COUNTS NO. OF PEOPLE WHO DIDN'T
    # HAVE TO WAIT. 
  return Stats
...
def Serving(Tills, NoOfTills, BuyerQ, QLength, Stats):
  TillFree = FindFreeTill(Tills, NoOfTills) # FINDS FREE TILL
  while TillFree != -1 and QLength > 0: # IF FREE TILL FOUND + QLENGTH IS > 0
    BuyerQ, QLength, BuyerID, WaitingTime, ItemsInBasket = ServeBuyer(BuyerQ, QLength) # FIRST BUYER SENT TO FREE TILL
  ...
  if QLength > 0:
    Stats[TOTAL_Q_OCCURRENCE] += 1  # count total number of buyers who do need to queue.!!!
    Stats[TOTAL_Q] += QLength  # sum's all Queue Length
...
def QueueSimulator():
  ...
  OutputHeading()
  TimeToNextArrival = Data[BuyerNumber + 1][ARRIVAL_TIME]  # extracts the time taken for FIRST buyer to arrive from the SimulatorData.txt file.
  for TimeUnit in range(SimulationTime):
    TimeToNextArrival -= 1
    print(f"{TimeUnit:>3d}", end='')  # end used to stop printing user ID on next line. Prints adjacent to TimeUnit.
