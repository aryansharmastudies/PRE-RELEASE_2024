...
def ChangeSettings():
    ...
    if Answer == 'Y':
        ...
        print(f"Current Optimization Status: {Optimization}")
...
def BuyerJoinsQ(Data, BuyerQ, QLength, BuyerNumber, Optimization):
    ...
    # TODO check the new persons ItemsinBasket.
    # TODO if it greater than first buyer, then SWAP.
    if Optimization == "ON" and ItemsInBasket > BuyerQ[0].ItemsInBasket:
      for x in range (QLength, -1, -1):
        BuyerQ[x + 1].BuyerID = BuyerQ[x].BuyerID
        BuyerQ[x + 1].ItemsInBasket = BuyerQ[x].ItemsInBasket
      BuyerQ[0].BuyerID = f"B{BuyerNumber}"
...
def BuyerArrives(Data, BuyerQ, QLength, BuyerNumber, NoOfTills, Stats, Optimization):
    ...
    BuyerQ, QLength = BuyerJoinsQ(Data, BuyerQ, QLength, BuyerNumber, Optimization)
...
def QueueSimulator():
    ...
    SimulationTime, NoOfTills, Optimization = ChangeSettings()
    ...
    for TimeUnit in range(SimulationTime):
        ...
        if TimeToNextArrival == 0:
            BuyerNumber += 1
            BuyerQ, QLength, NoOfTills, Stats = BuyerArrives(Data, BuyerQ, QLength, BuyerNumber, NoOfTills, Stats, Optimization)
            TimeToNextArrival = Data[BuyerNumber + 1][ARRIVAL_TIME]
        else:
            print()
