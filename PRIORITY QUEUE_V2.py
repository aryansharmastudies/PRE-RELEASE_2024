...
def ChangeSettings():
  ...
  if Answer == 'Y':
    ...
    Optimization = input("Enter 'TRUE' or 'FALSE'")
    while Optimization != "TRUE" or Optimization != "FALSE":
      print(f"Enter 'TRUE' or 'FALSE'")
      Optimization = input("Enter 'TRUE' or 'FALSE'")
...
def Serving(Tills, NoOfTills, BuyerQ, QLength, Stats, Optimization):  # 🟡🟡🟡
...
def QueueSimulator():
  ...
  for TimeUnit in range(SimulationTime):
    ...
    if TimeToNextArrival == 0:
      ...
      print()
    Tills, NoOfTills, BuyerQ, QLength, Stats = Serving(Tills, NoOfTills, BuyerQ, QLength, Stats, Optimization)
  ExtraTime = 0
  while QLength > 0:
